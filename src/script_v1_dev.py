...
def main():
    ...
    for cls, doc in data.items():
        words, pos, lemma = list(tokenization(
            doc, stopwords=stopwords, punctuations=punctuations))
        ...
        data_most_common_trigram_by_labels[cls] = most_common_words(
            trigramme(words), 5)

        # Entités nommées les plus fréquentes pour chaque
        # classe (PER-ORG-LOC uniquement)
        data_most_common_entity_by_labels[cls] = most_common_words(
            get_ner(doc), 5)

    # Longueur moyenne d’un document p/chaque classe
    data_avr_docs_len_by_labels = get_avr_len_doc_by_label(
        dev_labels, dev_corpus)
